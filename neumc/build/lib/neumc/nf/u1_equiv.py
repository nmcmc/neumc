...
class GenericGaugeEquivCouplingLayer(Flow):
    ...
    @override
    def reverse(self, fx: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def make_u1_equiv_layers(
    *,
    make_plaq_coupling: Callable[..., Flow],
    masks: Iterator[tuple[torch.Tensor, dict[str, torch.Tensor]]],
    n_layers: int,
    device,
    loops_function: Callable[[torch.Tensor], Sequence[torch.Tensor]] | None = None,
):
    ...
    return FlowSequence(layers).to(device)
