...
from neumc.nf.flow_abc import Flow, Flow


def sample(
    n_samples: int, batch_size: int, prior, layers: torch.nn.ModuleList | Flow
...
def log_prob(
    x: torch.Tensor, prior, layers: torch.nn.ModuleList | Flow
) -> torch.Tensor:
